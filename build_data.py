...
def mask_contact(text: str) -> str:
    """
    Mask any run of 5+ digits (phone numbers) down to just the last 4 digits,
    e.g. '081319989866' -> '••••••••9866'. Non-numeric text (IG usernames,
    LINE IDs, etc.) is left as-is since that's not sensitive the same way a
    phone number is. This matters because orders.json ships to every visitor's
    browser (this is a static site with no real backend) — repo is public,
    so raw phone numbers would otherwise be fully downloadable by anyone.
    """
    if not text:
        return text
    ...
    return re.sub(r"\d{5,}", mask_group, text)
...
def build():
    ...
    for _, row in df.iterrows():
        ...
        record = {
            ...
            "contact": mask_contact(str(row.get("ID LINE/WA/INSTAGRAM (AKTIF)", "")).strip()) if pd.notna(row.get("ID LINE/WA/INSTAGRAM (AKTIF)")) else "",
            "items": items,
